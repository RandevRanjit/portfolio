...
Each delta knows how to render itself (`~ Tempo: 128 → 135 BPM`, `+ Track added: "New Synth"`), and that string is both the CLI diff output and the auto-generated commit message.
...
```text
commit_file_to_branch (git/mod.rs:211)

  refs/heads/<file-branch>          HEAD     : untouched
             |                      index    : untouched
...
The architectural rule is strict: the daemon owns all VCS logic, and the CLI and the macOS app are thin IPC clients.
...
```text
            ...
            +--------------+--------------+
              ...
              v                         v
      +----------------+       +------------------+
      | clap CLI       |       | SwiftUI menubar  |
      | init / status  |       | app (1,664 LOC)  |
      | log / diff ... |       | repo cards, log  |
      +----------------+       +------------------+

     SaveDetected events broadcast to all clients
```
*Fig. 3 — one daemon owns the VCS logic; the CLI and menubar app are thin clients on the socket.*
...
This is Phase 1: local-only, Ableton-only. The boundary is deliberate, not hidden.
