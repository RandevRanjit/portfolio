...
- **Node/Express orchestrator** (`orchestrator/chat-server.js`) — the only thing that talks to OpenAI. It declares nine typed function schemas to `gpt-4o-mini` and runs the function-call → execute → synthesise round.
...
1. Sanitise the chat history (drop any message without string content; `gpt-4o-mini` rejects null-content function turns).
